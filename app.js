...
function downloadPhotoStrip() {
  ...
  stripCanvas.toBlob((blob) => {
    ...
    link.href = url;
    link.download = `handpuzzle_strip_${Date.now()}.png`;
...
async function initHandLandmarker() {
  ...
  try {
    vision = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Timed out loading the MediaPipe runtime (WASM). Check your internet connection or if cdn.jsdelivr.net is blocked.",
    );
  } catch (err) {
    throw err;
  }
  ...
  try {
    ...
    console.warn(
      "[Hand-Puzzle] GPU delegate failed, retrying with CPU…",
      gpuErr,
    );
  }
  ...
  try {
    const handLandmarker = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Timed out downloading the HandLandmarker model (~10MB) even with CPU. Check your connection or if storage.googleapis.com is blocked on your network.",
    );
    return handLandmarker;
  } catch (cpuErr) {
    throw cpuErr;
...
function isPinching(landmarks) {
  return (
    dist2D(landmarks[LM.THUMB_TIP], landmarks[LM.INDEX_TIP]) < PINCH_THRESHOLD
  );
}
...
function isFist(landmarks) {
  ...
  for (const [tipIdx, mcpIdx] of pairs) {
    if (dist2D(landmarks[tipIdx], wrist) < dist2D(landmarks[mcpIdx], wrist))
      curled++;
  }
  return curled >= 4;
}
...
function toPixel(landmarkNorm) {
  return {
    x: landmarkNorm.x * canvas.width,
    y: landmarkNorm.y * canvas.height,
  };
}
...
async function boot() {
  ...
  const watchdog = setTimeout(() => {
    if (!settled) {
      showLoaderError(
        "Loading is taking too long. Click retry or check your connection.",
      );
    }
  }, watchdogMs);
  ...
  try {
    ...
    if (err && err.name === "NotAllowedError") {
      showLoaderError(
        "Camera permission denied. Enable it in your browser settings and click retry.",
      );
